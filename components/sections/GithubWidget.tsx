...
async function getGithubStats(username: string) {
  try {
    ...
    const stats = repos.reduce((acc: any, repo: any) => {
      ...
      return acc;
    }, { stars: 0, forks: 0, repos: repos.length });
...
export default async function GithubWidget() {
  ...
  let stats = { stars: 0, forks: 0, repos: 0 };
  if (username) {
    const fetchedStats = await getGithubStats(username);
  ...
  return (
    <div className="md:col-span-3 border border-zinc-200 dark:border-white/5 bg-zinc-50 dark:bg-[#050505] rounded-[24px] p-5 flex flex-col transition-colors duration-300 min-h-[160px] relative overflow-hidden group">
      ...
      <div className="grid grid-cols-2 gap-4 mt-auto relative z-10">
        <div className="flex flex-col gap-1 border-l-2 border-zinc-200 dark:border-white/10 pl-3">
          <span className="flex items-center gap-1.5 text-zinc-500 dark:text-zinc-400 text-xs font-medium">
            <Star size={12} /> Stars
          </span>
          <span className="text-2xl font-bold text-zinc-900 dark:text-white font-sans">{stats.stars}</span>
        </div>
        
        <div className="flex flex-col gap-1 border-l-2 border-zinc-200 dark:border-white/10 pl-3">
          <span className="flex items-center gap-1.5 text-zinc-500 dark:text-zinc-400 text-xs font-medium">
            <GitCommit size={12} /> Repos
          </span>
          <span className="text-2xl font-bold text-zinc-900 dark:text-white font-sans">{stats.repos}</span>
        </div>
      </div>
    </div>
  );
